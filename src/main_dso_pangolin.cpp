...
void settingsDefault(int preset) {
  ...
  if (preset == 0 || preset == 1) {
    ...
    setting_kfGlobalWeight = 0.3; // original is 1.0. 0.3 is a balance between
                                  // speed and accuracy. if tracking lost, set
                                  // this para higher
    setting_maxShiftWeightT = 0.04f * (640 + 128); // original is 0.04f *
                                                   // (640+480); this para is
...
int main(int argc, char **argv) {
  ...
  std::thread runthread([&]() {
    ...
    for (int i = lstart;
         i >= 0 && i < reader_right->getNumImages() && linc * i < linc * lend;
         i += linc) {
      ...
      if (timesToPlayAtRight.size() == 0) {
        ...
        double tsThis =
            reader_right->getTimestamp(idsToPlay[idsToPlay.size() - 1]);
        double tsPrev =
            reader_right->getTimestamp(idsToPlay[idsToPlay.size() - 2]);
    ...
    for (int ii = 0; ii < (int)idsToPlay.size(); ii++) {
      ...
      bool MODE_SLAM = true;
      // if MODE_STEREOMATCH is true, it does stereo matching and output idepth
      // image.
      ...
      if (MODE_STEREOMATCH) {
        ...
        cv::Mat idepthMap(img_left->h, img_left->w, CV_32FC3,
                          cv::Scalar(0, 0, 0));
    ...
    // fullSystem->printResult("/home/davide/Libs/stereo-dso/build/result.txt");
