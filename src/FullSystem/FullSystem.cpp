...
Vec4 FullSystem::trackNewCoarse(FrameHessian *fh, FrameHessian *fh_right) {
  ...
  if (allFrameHistory.size() == 2) {
    ...
    for (float rotDelta = 0.02; rotDelta < 0.05; rotDelta = rotDelta + 0.02) {
      lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, rotDelta, 0, 0),
                                     Vec3(0, 0, 0))); // assume constant motion.
      lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, 0, rotDelta, 0),
                                     Vec3(0, 0, 0))); // assume constant motion.
      lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, 0, 0, rotDelta),
                                     Vec3(0, 0, 0))); // assume constant motion.
      lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, -rotDelta, 0, 0),
                                     Vec3(0, 0, 0))); // assume constant motion.
      lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, 0, -rotDelta, 0),
...
void FullSystem::stereoMatch(ImageAndExposure *image,
  ...
  for (ImmaturePoint *ph : fh->immaturePoints) {
    ...
    if (phTraceRightStatus == ImmaturePointStatus::IPS_GOOD) {
      ImmaturePoint *phRight = new ImmaturePoint(
          ph->lastTraceUV(0), ph->lastTraceUV(1), fh_right, &Hcalib);
      ...
      if (phTraceLeftStatus == ImmaturePointStatus::IPS_GOOD &&
          u_stereo_delta < 1 && depth > 0 &&
          depth < 70) // original u_stereo_delta 1 depth < 70
      {
        ph->idepth_min = ph->idepth_min_stereo;
        ph->idepth_max = ph->idepth_max_stereo;

        *((float *)(idepthMapPtr + int(ph->v) * idepthMap.step) +
          (int)ph->u * 3) = ph->idepth_stereo;
        *((float *)(idepthMapPtr + int(ph->v) * idepthMap.step) +
          (int)ph->u * 3 + 1) = ph->idepth_min;
        *((float *)(idepthMapPtr + int(ph->v) * idepthMap.step) +
          (int)ph->u * 3 + 2) = ph->idepth_max;
  ...
  //        std::cout << error[i].first << " " << error[i].second.first << " "
  //        << error[i].second.second << std::endl;

  std::cout << " frameID " << id << " got good matches " << counter
            << std::endl;
    ...
    for (ImmaturePoint *ph : host->immaturePoints) {
      ...
      if (phTrackStatus == ImmaturePointStatus::IPS_GOOD) {
        ...
        ImmaturePointStatus phNonKeyStereoStatus =
            phNonKey->traceStereo(fh_right, K, 1);
        ...
        if (phNonKeyStereoStatus == ImmaturePointStatus::IPS_GOOD) {
          ...
          ImmaturePointStatus phNonKeyRightStereoStatus =
              phNonKeyRight->traceStereo(fh, K, 0);
