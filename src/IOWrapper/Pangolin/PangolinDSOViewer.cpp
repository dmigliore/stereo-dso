...
void PangolinDSOViewer::run() {
  ...
  pangolin::View &d_video_Right =
      pangolin::Display("imgKFDepth_Right").SetAspect(w / (float)h);
  ...
  pangolin::GlTexture texVideo(w, h, GL_RGB, false, 0, GL_RGB,
                               GL_UNSIGNED_BYTE);
  pangolin::GlTexture texVideo_Right(w, h, GL_RGB, false, 0, GL_RGB,
                                     GL_UNSIGNED_BYTE);
  ...
  // show ground truth
  std::cerr << "Starting loading groudtruth" << std::endl;
  std::string gtPath = "/home/davide/Desktop/link_kitti/dataset/poses/00.txt";
  ...
  while (!pangolin::ShouldQuit() && running) {
    ...
    if (videoImgChanged) {
      ...
      texVideo_Right.Upload(internalVideoImg_Right->data, GL_BGR,
                            GL_UNSIGNED_BYTE);
    }
    if (kfImgChanged) {
...
void PangolinDSOViewer::pushStereoLiveFrame(FrameHessian *image,
  ...
  for (int i = 0; i < w * h; i++) {
    internalVideoImg->data[i][0] = internalVideoImg->data[i][1] =
        internalVideoImg->data[i][2] =
            image->dI[i][0] * 0.8 > 255.0f ? 255.0 : image->dI[i][0] * 0.8;
    internalVideoImg_Right->data[i][0] = internalVideoImg_Right->data[i][1] =
        internalVideoImg_Right->data[i][2] =
            image_right->dI[i][0] * 0.8 > 255.0f ? 255.0
                                                 : image_right->dI[i][0] * 0.8;
  }
...
void PangolinDSOViewer::pushDepthImage(MinimalImageB3 *image) {
  ...
  struct timeval time_now;
  gettimeofday(&time_now, NULL);
  lastNMappingMs.push_back(((time_now.tv_sec - last_map.tv_sec) * 1000.0f +
